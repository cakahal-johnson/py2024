...
# THIS ROUTE IS JUST FOR STATIC VIEW WITHOUT THE DATABASE
...
#     per_page=4)
    ...
    # return render_template('products/index.html', categories=categories(), brands=brands(), products=products)


# @app.route('/product/<int:id>')
# def single_page(id):
#     product = Addproduct.query.get_or_404(id)
#     return render_template('products/single_page.html', brands=brands(), categories=categories(), product=product)
...
def get_brand(id):
    # get_b = Category.query.filter_by(id=id).first_or_404()
    # page = request.args.get('page', 1, type=int)
    # brand= Addproduct.query.filter_by(brand_get_b).paginate(page=page, per_page=4)
    # return render_template('products/index.html', get_b=get_b, categories=categories(), brands=brands(), brand=brand)
...
def get_category(id):
    ...
    # get_cat_prod = Addproduct.query.filter_by(category_get_cat).paginate(page=page, per_page=4)
    # return render_template('products/index.html', brands=brands(), categories=categories(), get_cat_prod=get_cat_prod,
    # get_cat=get_cat)
    return render_template('products/index.html')
...
#         stock = form.data.stock.data
#         colors = form.data.color.data
#         brand = request.form.get('brand')
#         category = request.form.get('category')
...
#
#         addpro = Addproduct(name, price=price, discount, stock=stock, colors=colors, desc=desc,
#         brand_id=brand, category_id=category, image_1=image_1, image_2=image_2, image_3=image_3)
#         db.session.add(addpro)
#         flash(f'Te product {name} has been added to your database', 'success')
...
#    return render_template('products/updateproduct.html',product=product,brands=brands,categories=categories,form=form)
...
def updateproduct(id):
    ...
    # THIS IS TO WORK WITHOUT DB SESSION
